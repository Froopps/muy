<?php
    ...
    if($_FILES["cropped_pro_pic"]["error"]>0){
        $redirect_with_error.=urlencode("Errore nel caricamento della foto profilo. Assegnata quella di default");
    ...
    $pro_pic=$_SERVER["DOCUMENT_ROOT"]."/../muy_res/content/".$_POST["user"]."/".$_FILES["cropped_pro_pic"]["name"];
    $q_pro_pic=escape("content/".$_POST["user"]."/".$_FILES["cropped_pro_pic"]["name"],$connected_db);
    $query="UPDATE utente SET foto='".$q_pro_pic."' WHERE email='".escape($_POST["user"],$connected_db)."'";
    $connected_db->query($query);
