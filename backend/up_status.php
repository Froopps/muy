<?php
    ...
    switch ($_POST['action']){
        case 'accept':
            ...
            if($res!='pending')
                goto error;
            $query="UPDATE amicizia SET stato='a', dataRisposta='".date('Y-m-d',time())."' WHERE sender='$object' AND receiver='$subject'";
        ...
        case 'deny':
            ...
            if($res!='pending')
                goto error;
            $query="UPDATE amicizia SET stato='r', dataRisposta='".date('Y-m-d',time())."' WHERE sender='$object' AND receiver='$subject'";
            $res=$connected_db->query($query);
            if(!$res){
                log_into("Errore di esecuzione della query ".$query." ".$connected_db->error);
                goto error;
        ...
        case 'erase':
            ...
            if($res!='a')
                goto error;
            $query="UPDATE amicizia SET stato='p' WHERE sender='$subject' AND receiver='$object' OR sender='$object' AND receiver='$subject'";
            $res=$connected_db->query($query);
            if(!$res){
                log_into("Errore di esecuzione della query ".$query." ".$connected_db->error);
                goto error;
